...
#define FONT_NAME(N,F) [UIFont fontWithName:N size:F]
...
//--------------------------------------------------
//更新UI需回到主线程
static inline dispatch_source_t gcdTimer(NSTimeInterval delay,NSUInteger times, dispatch_block_t block)
{
    if (block) {
        __block NSInteger index = 0;
        dispatch_source_t timer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0));
        dispatch_source_set_timer(timer, DISPATCH_TIME_NOW, delay * NSEC_PER_SEC, 0.0);
...
#define GCD_TIMER(delayTimeInterval,times,block) gcdTimer(delayTimeInterval,times,block);
#define GCD_TIMER_CANCEL(timer) dispatch_cancel(timer);
...
#define GCD_AFTER(timeInterval,block) dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(timeInterval * NSEC_PER_SEC)), dispatch_get_main_queue(), block);
/*
 --------------------
 GCD_MAIN(^{
 NSLog(@"主线程输出");
...
--------------------
 GCD_TIMER(0.3,5,^{
 NSLog(@"以0.3s的间隔，输出5次(0次才表示无限次，需手动取消)");
 })
 --------------------
 */
